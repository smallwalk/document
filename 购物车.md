...
<center>![购物车列表业务结构图](img/购物车列表业务结构图.png)</center>
<center>图2-1：购物车列表业务结构图</center>

从上面结构图来看购物车的列表主要包括两个大部分，这个是由购物车承载的功能点来设计。从目前线上的设计缺陷来分析以下这样设计的目的，目前线上购物车列表的各模块之前的区分不是很明确，虽然能大体区分出这个模块，但代码是杂糅在一起的，其中暂存功能中的商品模块和优惠展示中的活动数据是在一起提供的，Cart数据的获取又嵌入了商品模块数据的获取（**嵌入购物车的读取model里面，相当于商品是购物车的一部分**），这样设计的初衷是在引入sku的时候，减少了很大部分的工作量，但是缺点也是显而易见，一是如果商品模块出问题，购物车其他不需要这个模块的业务也会受到影响，二是核心数据和非核心数据（活动）杂糅在一起，业务的迭代开发比较困难，并且在线上压力大的时候没有办法做降级处理。
